  ...
  return (
    <div className="grid lg:grid-cols-5 gap-5">
      <div className="lg:col-span-3 bg-white rounded-2xl border border-navy/10 p-5">
        <h3 className="font-display text-lg mb-1">Your data, your controls</h3>
          ...
          {consentItems.map((c) => (
            <div key={c.key} className="flex items-start justify-between gap-4 py-3.5">
              <div>
                ...
                <div className="text-xs text-navy/50 mt-0.5">{c.detail}</div>
              </div>
              <button
                role="switch"
                ...
                className={`shrink-0 w-11 h-6 rounded-full transition-colors relative ${
                  consents[c.key] ? "bg-green" : "bg-navy/15"
                }`}
              >
                <span
      ...
      <div className="lg:col-span-2 space-y-5">
        <div className="bg-navy rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-display text-base text-paper">DPDP Act alignment</h3>
          </div>
          <ul className="space-y-2.5">
            {dpdpPoints.map((p) => (
              <li key={p} className="text-xs text-paper/70 leading-relaxed flex gap-2">
        ...
        <div className="bg-white rounded-2xl border border-navy/10 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-display text-base">Guarding against AI error</h3>
          </div>
          <ul className="space-y-2.5">
            {safetyPoints.map((p) => (
              <li key={p} className="text-xs text-navy/60 leading-relaxed flex gap-2">
